...
public class ShortcutChooseAdapter extends RecyclerView.Adapter<ShortcutChooseAdapter.ViewHolder> {

    private Activity context;
    private ArrayList<App> apps;
    private AppData appData;

    public ShortcutChooseAdapter(Activity context, ArrayList<App> apps, AppData appData) {
    ...
    @NonNull
    @Override
    public ShortcutChooseAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new ViewHolder(LayoutInflater.from(context).inflate(R.layout.item_app_shortcut_main,parent,false));
    }

    @Override
    public void onBindViewHolder(@NonNull ShortcutChooseAdapter.ViewHolder holder, final int position) {
        App app = apps.get(position);
        if(appData.getIcon(app.getResolveInfo())!=null){
            Glide.with(context).load(appData.getIcon(app.getResolveInfo()))
                    .into(holder.imgIcon);
        }
        app.setLoadAppCallback3(() -> context.runOnUiThread(() -> notifyItemChanged(position)));
    }
